...
class UserJadwalRapatNotulensiEdit extends Component
{
    public $judul_rapat, $kategori_rapat, $topik_rapat,
        ...
        $status_rapat, $rapat_id, $team, $team_nama, $rapat_slug, $hasil_rapat, $catatan, $notulensi;

    public function render()
    {
        return view('livewire.user-jadwal-rapat-notulensi-edit', [
    ...
    {
            $this_notulensi = Notulensi::find(Notulensi::where('id_rapat', $this->rapat_id)->first()->id);
            $this_notulensi->update([
                'hasil_rapat' => $this->hasil_rapat,
                'catatan' => $this->catatan,
                'updated_by' => auth()->user()->id
            ]);
        $this->resetInput();
        return redirect()->route('jadwal-rapat.show', ['team' => $this->team, 'rapat' => $this->rapat_slug])->with('message', 'Notulensi Rapat ' . $this->old_judul_rapat . ' telah diubah !');
    }
}
